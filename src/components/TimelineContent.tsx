...
export function TimelineContent({ onKitClick, scrollContainerRef }: TimelineContentProps) {
  ...
  // Track which year is visible based on scroll inside .content container
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // The stats row + year nav height (sticky panels at top of .content)
      const panelsHeight = (statsRowRef.current?.offsetHeight || 44) + (yearNavRef.current?.offsetHeight || 48);
      const topThreshold = panelsHeight + 10;
  ...
  }, [loading, timelineData, viewMode, selectedYear, scrollContainerRef]);
  ...
  // Scroll margin = sticky panels height inside .content
  const scrollMarginTop = 100;

  return (
    <div className="w-full">
      {/* Sticky Stats Row - sticks at top of .content scroll container */}
      <div
        ref={statsRowRef}
        className="sticky top-0 z-20 bg-black/70 backdrop-blur-md border-b border-white/10"
      >
          ...
          <div className="flex items-center gap-1 bg-black/60 rounded-lg p-1">
            <button
              ...
              className={`h-7 px-3 text-xs font-medium rounded-md transition-all ${
                viewMode === 'timeline'
                  ? 'bg-[#cd2127] text-white shadow-sm'
      ...
      {/* Sticky Year Navigation - sticks below stats row */}
      {timelineData.length > 0 && (
        <div
          ref={yearNavRef}
          className="sticky z-10 bg-black/70 backdrop-blur-md border-b border-white/10 overflow-x-auto"
          style={{ top: statsRowRef.current?.offsetHeight || 44 }}
        >
          <div className="flex items-center gap-1 py-2">
            {timelineData.map((group) => (
              <button
                ...
                className={`shrink-0 h-8 px-3 text-xs font-medium rounded-md transition-all cursor-pointer border ${
          ...
          {timelineData.map((group) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-4 -ml-1">
                <div className="relative z-10 w-[60px] h-[60px] rounded-full bg-gradient-to-br from-[#002f42] to-[#004d6d] flex items-center justify-center text-white font-bold text-sm shadow-lg ring-4 ring-black/40">
                  {group.year}
                </div>
                <div>
